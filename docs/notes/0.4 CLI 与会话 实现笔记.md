...
- `--thinking` 开启 Extended Thinking（0.5 实现）
...
这里可以简单做个测试：
```shell
node mock/mock-llm.mjs  # 得到mock服务地址
...
这里介绍下大概的实现：
- parseArgs
  - 添加 deleteSessionId 的设置
  - 冲突检查："--delete-session 不能和 --continue 或 --resume 同时使用"
- runCli 里面的相关处理，前提是判断已经使用了 `--delete-session`
  - 有 `deleteSessionId` 直接删除指定 id
  - 进入交互选择，让用户指定删除的 id
  - 报错兜底

定义的几个方法说明下：
  - `confirmSessionDeletion` 确认会话删除，需要用户通过 `y/yes` 进行确认
  - `selectSessionToDelete` 交互，选择会话进行删除
  - `printSessionList` 打印会话列表，基于 `getSessionPreview` 做的简单封装，方便 删除和恢复打印 复用
  - `isConfirmed` 对用户输入进行 `input.trim().toLowerCase()` 处理，判断是否是 `y` 或 `yes`
  - `deleteSession` 调用 nodeJS 底层方法删除指定 id 的文件
