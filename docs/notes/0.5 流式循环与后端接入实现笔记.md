...
下面是第 4 阶段完成时的实现快照，此时还没有加入第 6 阶段的统一中断。最终的中断感知版本见第 6 节。
...
最终的 `executeBlock()` 会在执行前检查 Signal，并将执行期间由 Signal 导致的失败转换成 `interrupted`：
...
这里必须在 `try` 内 `await this.execute(...)`。如果只返回 Promise，`try` 会在 Promise 被拒绝前结束，后面的 `catch` 无法捕获异步错误，也就不能把由 Signal 引起的失败识别为 `interrupted`。

`finish()` 仍按原始 `tool_use` 顺序遍历：
