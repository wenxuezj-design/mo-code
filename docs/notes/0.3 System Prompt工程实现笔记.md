...
这一阶段围绕 mo-code 的 System Prompt 工程展开，将 Agent 运行所需的信息拆分为静态行为规则、动态环境上下文和项目级指令。具体实现包括静态 Prompt 模板、环境与 Git 信息组装，以及对 `CLAUDE.md`、Rules 和 `@include` 的加载，让 Agent 在进入对话循环前能够了解自身职责、当前运行环境和项目约定。

我们参考 Claude Code（下文简称 CC）公开的工作方式和项目指令机制，重点对齐信息分层、Prompt 缓存边界和项目上下文注入方式，不复刻其内部 System Prompt。当前版本优先实现可理解、可测试的核心链路，并通过目录边界、引用深度和读取失败降级限制项目指令的加载范围；Memory、Skills 和 Subagent 等动态能力暂时只保留接口，后续再逐步补充。
...
```mermaid
flowchart TD
  PATHS["isPathWithinRoot()"] --> INCLUDES["resolveIncludes()"]
  PATHS --> CLAUDE["loadClaudeMd()"]
  PATHS --> RULES["loadRulesDir()"]
  INCLUDES --> CLAUDE
  INCLUDES --> RULES
  CLAUDE --> PROJECT["loadProjectInstructions()"]
  RULES --> PROJECT
...
export function loadProjectInstructions(): string {
  ...
  while (true) {
    ...
    // 从当前目录向上级遍历，直到根目录停止，因为根目录的上级查出来还是根目录
...
function loadRulesDir(dir: string): string[] {
  ...
  try {
    const allowedRoot = realpathSync(dir);
    const files = globSync("**/*.md", { cwd: rulesDir, nodir: true }).sort(); //先排序
    const parts: string[] = [];
...
> 项目外引用默认阻止。`CLAUDE.md` 的允许范围是它所在目录。Rules 的允许范围是拥有 `.claude` 的目录。引用文件继承该范围。
>
> 路径使用 `realpath()` 后再判断，防止符号链接越界。Claude Code 会对项目外引用进行首次授权。初版不实现该交互，后续可以扩展，见 [Claude Code：导入其他文件](https://code.claude.com/docs/en/memory#import-additional-files)。
...
- 防护处理
  - visited Set 防止循环引用
  - MAX_INCLUDE_DEPTH 放置嵌套过深
  - 路径防护：防止超过根目录
  - 优雅兜底：异常时会留下 HTML 注释标记，不报错中断
...
const MAX_INCLUDE_DEPTH = 4; // 和 cc 官方保持一致

// 判断目标文件是否在指定的根目录中
function isPathWithinRoot(rootPath: string, targetPath: string): boolean {
  const pathFromRoot = relative(rootPath, targetPath);

  if (pathFromRoot === "") return true; // 目标就是根目录，允许
  if (isAbsolute(pathFromRoot)) return false; // 跨磁盘情况，会返回比如说 D:\file，拒绝
  if (pathFromRoot === "..") return false; // 目标正好是根目录的父目录，拒绝
  if (pathFromRoot.startsWith(`..${sep}`)) return false; // 以 .. 表明是父级及以上目录，拒绝，这里的 sep 是系统分隔符，在 windows 和 mac 系统是不同的
...
function resolveIncludes(
  content: string, // 内容
  basePath: string, // 基础
  allowedRoot: string, // 允许的根目录路径
  visited: Set<string> = new Set(), // 已经访问过的内容
  depth: number = 0 // 深度
): string {
  if (depth >= MAX_INCLUDE_DEPTH) return content; // 和cc保持一致，超过最大层级直接返回
  ...
  return content.replace(INCLUDE_REGEX, (_match, rawPath: string) => {
    ...
    resolved = resolve(resolved); // 得到规范的绝对路径写法，比如 /project/docs/../README.md 会转为 /project/README.md
...
> 当前为初版实现。之后继续做的话，会解决以下边界问题：
>
> - `visited` 可能把重复引用误判为循环引用。
> - Markdown 代码块中的独占一行 `@` 也会被解析。
> - 暂未限制文件数量、单文件大小和总字符数。达到深度上限后会保留原始 `@` 引用。
>
> 初版只支持独占一行的 `@` 引用。Claude Code 还支持内联引用，并会跳过代码块，见 [Claude Code：导入其他文件](https://code.claude.com/docs/en/memory#import-additional-files)。
